...
export default function MicroappStub() {
  return (
    <div className="grid gap-8 md:grid-cols-2 lg:max-w-6xl mx-auto w-full p-4 md:p-8">
      <Card>
        <CardHeader>
          <CardTitle>Testimonial Rewriter</CardTitle>
          <CardDescription>Polish up rough user reviews.</CardDescription>
        </CardHeader>
        <CardContent>
          <form action={async (formData) => {
            "use server";
            // Local stub logic: Deterministic DummyProvider output
            console.log("Generating test-rewrite output...");
          }} className="space-y-4">

            <div className="space-y-2">
              <Label htmlFor="review">Raw Review</Label>
      ...
      <Card className="bg-muted/50">
        <CardHeader>
          ...
          <CardDescription>Expect: A polished, marketing-ready testimonial</CardDescription>
        </CardHeader>
